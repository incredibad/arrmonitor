...
function Stat({ label, value, color, bg, active, always }) {
  const on = always || active;
  return (
    <div className={styles.stat} style={on ? { background: bg } : undefined}>
      <span className={styles.statVal} style={on ? { color } : undefined}>{value}</span>
      <span className={styles.statLabel} style={on ? { color, opacity: 1 } : undefined}>{label}</span>
    </div>
  );
}
